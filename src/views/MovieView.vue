...
<template>
    <h1 class="text-center text-6xl font-bold">Ultimos Estrenos de las películas</h1>
    <header
      class="w-full h-22 relative h-full md:h-[400px] lg:h-[600px]"
      v-if="movieStore.movies"
    >
      <img
        class="w-full opacity-40 h-auto lg:h-full absolute -z-50"
        :src="`https://image.tmdb.org/t/p/original${movieStore.movie.backdrop_path}`"
        alt=""
      />
      <div
        class="z-20 px-10 lg:px-32 py-10 flex flex-col lg:flex-row gap-5 lg:gap-20"
      >
        <div class="flex items-center mx-h-[570px]">
          <img
            class="w-auto max-h-[570px] mx-auto lg:mx-0 rounded-xl"
            :src="`https://image.tmdb.org/t/p/original${movieStore.movie.poster_path}`"
            alt=""
          />
        </div>
        <div class="mt-10">
    ...
    <section class="max-w-7xl mx-auto lg:p-0 mt-10 md:mt-[600px] lg:mt-10 p-5">
      <h2 class="text-white text-2xl font-semibold text-center">Reparto principal</h2>
      <div class="flex flex-row flex-wrap justify-center gap-5 mt-5">
        <div class="w-20" v-for="item in movieStore.sliceCast" :key="item.id">
          <img
            class="rounded-lg"
